...
function fetchWithIPandCreateDom() {
  getForecastIP().then(function (result) {
    forecast = result;
    createDOM(0);
  });
}
...
function getWithGeolocation() {
  geolocation = getGeolocation().then(function (result) {
    geolocation = result;
    if (
      geolocation.address.city == undefined &&
      geolocation.address.village != undefined
    ) {
...
function fetchWithCityAndCreateDom() {
  getForecast().then(function (result) {
    forecast = result;
...
async function getGeolocation() {
  try {
    const response = await fetch(
      `https://us1.locationiq.com/v1/reverse?key=pk.9fdc1490ae4d809ce9ece84b2c3bda46&lat=${latitude}&lon=${longitude}&format=json&accept-language=en&`,
      { mode: "cors" }
    );
...
async function getForecast() {
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/forecast.json?key=b97a7289e4e24edbbb8101327242006&q=${city}&days=8&aqi=yes`,
      { mode: "cors" }
    );
    if (response.ok == true) {
...
export { getLocation, searchLocation, forecast };
